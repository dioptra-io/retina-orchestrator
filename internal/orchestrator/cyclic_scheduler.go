...
// CyclicScheduler implements the responsible probing algorithm. It schedules
...
func (s *CyclicScheduler) NextPD() (*api.ProbingDirective, error) {
	s.mutex.Lock()
...
func (s *CyclicScheduler) UpdateFromFIE(fie *api.ForwardingInfoElement) error {
	s.mutex.Lock()
...
func (s *CyclicScheduler) recordImpact(address net.IP, pd *pdState) {
	if address == nil {
		return
	}
	key := ipKey(address)
...
func (s *CyclicScheduler) publishEvent(event SSEEvent) {
...
func (s *CyclicScheduler) removeImpact(address net.IP, pd *pdState) {
	if address == nil {
		return
	}
	key := ipKey(address)
