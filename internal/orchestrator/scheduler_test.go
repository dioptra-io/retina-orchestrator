...
func newTestScheduler(t *testing.T, pds []*api.ProbingDirective) *Scheduler {
	t.Helper()
	s, err := NewScheduler(42, 1000.0, writeSchedulerPDFile(t, pds), 0, testLogger(), testMetrics())
	if err != nil {
...
func TestNewScheduler_InvalidRate(t *testing.T) {
	...
	for _, rate := range []float64{0, -1} {
		_, err := NewScheduler(0, rate, "irrelevant", 0, nil, testMetrics())
		if err == nil {
...
func TestNewScheduler_MissingFile(t *testing.T) {
	t.Parallel()
	_, err := NewScheduler(0, 1.0, "/nonexistent/path.jsonl", 0, testLogger(), testMetrics())
	if err == nil {
...
func TestNewScheduler_EmptyFile(t *testing.T) {
	t.Parallel()
	_, err := NewScheduler(0, 1.0, writeSchedulerPDFile(t, nil), 0, testLogger(), testMetrics())
	if err == nil {
...
func TestNewScheduler_NilLogger(t *testing.T) {
	t.Parallel()
	s, err := NewScheduler(0, 1.0, writeSchedulerPDFile(t, []*api.ProbingDirective{makePD(1)}), 0, nil, testMetrics())
	if err != nil {
...
func TestReadPDs_InvalidJSON(t *testing.T) {
	...
	_, err = NewScheduler(0, 1.0, f.Name(), 0, testLogger(), testMetrics())
	if err == nil {
