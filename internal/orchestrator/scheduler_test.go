...
func TestUpdateFromFIE_TimeoutClearsAddress(t *testing.T) {
	t.Parallel()

	tests := []struct {
		name       string
		addr       net.IP
		makeFirst  func(net.IP) *api.ForwardingInfoElement
		recordType string
	}{
		{
			name: "near",
			addr: net.ParseIP("10.0.0.1"),
			makeFirst: func(addr net.IP) *api.ForwardingInfoElement {
				return makeFIE(1, addr, nil)
			},
			recordType: "near",
		},
		{
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := s.UpdateFromFIE(tt.makeFirst(tt.addr)); err != nil {
				t.Fatalf("unexpected error: %v", err)
			}

			if _, ok := s.impactRecords[ipKey(tt.addr)]; !ok {
				t.Fatalf("expected impact record for %s address", tt.recordType)
			}

			// Second FIE: timeout, stale address must be cleared.
			if err := s.UpdateFromFIE(makeFIE(1, nil, nil)); err != nil {
				t.Fatalf("unexpected error: %v", err)
			}
			...
			if _, ok := s.impactRecords[ipKey(tt.addr)]; ok {
				t.Fatalf(
					"expected stale %s address impact record to be removed on timeout",
					tt.recordType,
				)
			}
			...
			if s.pdMap[1].issuanceProb != 1.0 {
				t.Fatalf(
					"expected issuance prob 1.0 after timeout, got %v",
					s.pdMap[1].issuanceProb,
				)
			}
		})
	}
